...
### Nonce for countering attacks
nonce_pool = {}
class Nonce(pkt):
    name = "Nonce"
    fields_desc = [IntField("nonce", self.getNonce()), IntField("reply", 0)]

    def getNonce():
    nonce = random.getrandbits(32)
    # add nonce to the pool, increment count if already exists at key
    self.nonce_pool[nonce] = self.nonce_pool.get(nonce, 0) + 1 
    return nonce

bind_layers(TCP, Nonce, sport=SRC_PORT)
bind_layers(TCP, Nonce, dport=DST_PORT)

def checkNonce(pkt):
    if not pkt.haslayer(Nonce):
        return
    else:
        # check if nonce reply matches
        nonce_reply = pkt[Nonce].reply
        nonce_count = nonce_pool.get(nonce_reply, 0)
        if nonce_count == 0:
            return
        else:
            if nonce_cnt == 1:
                del nonce_pool[nonce_reply]
            else:
                nonce_pool[nonce_reply] = nonce_cnt - 1
...
## Sniffer runs in background constantly to check if packets have nonces
class Sniffer(Thread):
    def  __init__(self, interface="client-eth0"):
        super().__init__()
        self.interface = interface
        self.stop_sniffer = Event()

    def run(self):
        # only sniff packets coming to the sender, ie acks
        sniff(iface=self.interface, prn=checkNonce, filter="dst port {}".format(SRC_PORT), stop_filter=self.should_stop_sniffer)
